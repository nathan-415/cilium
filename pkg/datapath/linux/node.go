...
func (n *linuxNodeHandler) NodeCleanNeighbors(migrateOnly bool) {
	...
	for _, neigh := range neighList {
		...
		if migrateOnly {
			...
			// state entires with NTF_EXT_LEARNED.
			if (neigh.State & netlink.NUD_PERMANENT) == 0 {
				continue
			}
	...
	if neighSucceeded != 0 {
		...
		}).Infof("Successfully %s non-GC'ed ARP entries previously installed by cilium-agent", which)
	}
	if neighErrored != 0 {
		log.WithFields(logrus.Fields{
